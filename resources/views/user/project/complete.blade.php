...
<div class="property-wrap wow fadeInUp">
  ...
  @php
    $type=App\Model\Subcategory::skip(2)->first();
  @endphp
  <div id="London" class="tabcontent">
    <!--Row Start-->
    @php
      $complete=App\Model\Property::leftjoin('categories','properties.category_id','categories.id')->leftjoin('popular_places','properties.location_id','popular_places.id')->select('properties.*','categories.category_name','popular_places.name')->where('type_id',$type->id)->orderBy('id','DESC')->paginate(12);
    @endphp
    <ul class="row">
    ...
    <!--Row End-->
    <div class="float-right text-center">{{ $complete->links() }}</div>
  </div>
